...
class BounceLinkGateway @Autowired constructor(private val jdbcTemplate: JdbcTemplate) {
    ...
    fun findByKey(key: String): BounceLink? {
        val links: List<BounceLink>? = jdbcTemplate.query(SELECT_LINK_BY_NAME, arrayOf(key), ::mapLink)
    ...
    private fun mapLink(rs: ResultSet, recordNum: Int): BounceLink {
        return BounceLink(
    ...
    fun findAll(): List<BounceLink> {
        return jdbcTemplate.query(FIND_ALL, ::mapLink)
    }
